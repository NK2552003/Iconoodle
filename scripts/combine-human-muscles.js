...
(async ()=>{
  ...
  const files = (await fs.readdir(dir)).filter(f => /^human-muscular-system-muscle-.*\.json$/.test(f));
  if(files.length === 0){
    console.error('No matching files found in', dir);
    process.exit(1);
  }
  const combined = [];
  ...
  const outFile = path.join(dir, 'human-muscular-system-muscles.json');
  await fs.writeFile(outFile, JSON.stringify(combined, null, 2));
  console.log('Wrote', outFile, 'items:', combined.length);
})();